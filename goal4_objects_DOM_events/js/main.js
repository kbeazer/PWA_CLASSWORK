...
console.log('------ STUDENT ACTIVITY - ANSWERS BELOW ----------');
    ...
    console.log(objectTest.schoolName, objectTest['schoolName']);
    var newCnt= objectTest.studentCount;
    console.log(newCnt);
    console.log(objectTest['address']);
    console.log(objectTest.students[0].GPA, objectTest['students'][0]['GPA']);
...
----------------------------------------------------------------------------- */
function gradeAvg(average){
    var count= 0;
    var total= 0;
    for(var i=0;i<objectTest.students.length;i++){
        count++;
        total= total+objectTest.students[i]['GPA'];
    }
    return total/count;
} console.log(gradeAvg(objectTest))
